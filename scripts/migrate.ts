...
async function migrateProductInventoryRules() {
  const now=new Date();
  for(const configuration of getDefaultProductCatalog()){
    const { sku, ...flags } = configuration;
    await database.collection("products").updateOne(
      { sku },
      {
        $set: {
          ...flags,
          retailRatePaisa: Long.fromNumber(configuration.retailRatePaisa ?? 0),
          lowStockMilli: Long.fromNumber(configuration.lowStockMilli ?? 0),
          stockMilli: Long.ZERO,
          averageCostPaisa: Long.ZERO,
          updatedAt: now,
        },
        $setOnInsert: { sku, createdAt: now },
      },
      { upsert: true },
    );
  }
