...
export async function dashboard(from?: string, to?: string) {
  ...
  const [purchases, sales, expenses, receivables, payables, alerts, milkFlow, deliveryProgress, expectedMilk, dailySales, dailyPurchases, dailyExpenses, customerBalances, vendorBalances, revenueComposition, expenseCategories, stock, production] = await Promise.all([
    ...
    database.collection("financial_transactions").aggregate([{ $match: { ...dateMatch, kind: { $in: ["sale", "customer_delivery"] }, status: "posted" } }, { $group: { _id: "$businessDate", amount: { $sum: "$amountPaisa" } } }, { $sort: { _id: 1 } }]).toArray(),
    ...
    database.collection("financial_transactions").aggregate([{ $match: { ...dateMatch, kind: { $in: ["sale", "customer_delivery"] }, status: "posted" } }, { $group: { _id: "$kind", amount: { $sum: "$amountPaisa" } } }]).toArray(),
  ...
  return { businessDate, purchases, sales, expenses, receivables, payables, alerts, milkFlow, deliveryProgress, expectedMilk, dailySales, dailyPurchases, dailyExpenses, customerBalances, vendorBalances, revenueComposition, expenseCategories, stock, production, refreshedAt: new Date() };
}
