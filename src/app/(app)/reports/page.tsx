...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{from?:string;to?:string;tab?:string;q?:string}> }) {
  ...
  const dailyCollections = summary.dailyCustomerPayments.map(row=>({name:String(row._id).slice(5),collections:paisa(row.amount)}));
  ...
  return <div className="content">
    ...
    {tab==="overview" && <><SectionHeader title="Business overview" description="A compact summary for the selected period."/><div className="executive-grid">
      ...
      <MetricCard label="Customer collections" value={formatPKR(big(summary.customerPayments?.amount))} note={`${Number(big(summary.customerPayments?.count))} payments`} tone="brand"/>
    ...
    {tab==="customers" && <><SectionHeader title="Customer analysis" description="Collection priorities and delivery completion."/><div className="chart-grid"><ChartCard title="Highest outstanding balances" description="Current receivables by customer." summary={`${summary.customerBalances.length} customers shown.`}><CompositionChart data={summary.customerBalances.map(row=>({name:String(row.name),value:paisa(row.balance)}))} moneyValues/></ChartCard><ChartCard title="Customer collections" description="Household and shop customer payments received." summary={`${Number(big(summary.customerPayments?.count))} payments in this period.`}>{dailyCollections.length?<TrendChart data={dailyCollections} keys={[{key:"collections",label:"Collections",color:"#16a34a"}]} moneyValues/>:<EmptyState title="No collections" description="Customer payments will appear here."/ >}</ChartCard><ChartCard title="Delivery completion" description="Households served in the selected period." summary={`${Number(big(summary.deliveryProgress?.delivered))} delivered.`}><CompositionChart data={[{name:"Delivered",value:Number(big(summary.deliveryProgress?.delivered))},{name:"Skipped",value:Number(big(summary.deliveryProgress?.skipped))}]}/></ChartCard></div></>}
