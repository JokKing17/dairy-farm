...
export default async function Page(){
 ...
 return <div className="content">
  <div className="customer-heading"><div><div className="title">Shop Sales</div><div className="subtitle">Fast Paid Now and Credit/Udhaar sales. Walk-in Cash sales need no customer.</div></div><ShopSaleForm today={today} products={products.map(product=>({sku:String(product.sku),name:String(product.name),unit:String(product.unit),ratePaisa:integerToBigInt(product.retailRatePaisa).toString(),stockMilli:integerToBigInt(product.stockMilli).toString(),piecesPerTray:product.sku==="EGG-001"?Number(product.piecesPerTray??30):undefined,pieceRatePaisa:product.sku==="EGG-001"?integerToBigInt(product.pieceSellingRatePaisa,product.retailRatePaisa).toString():undefined,trayRatePaisa:product.sku==="EGG-001"?integerToBigInt(product.traySellingRatePaisa).toString():undefined,defaultSaleUnit:product.sku==="EGG-001"&&product.defaultSaleUnit==="tray"?"tray":"piece"}))} customers={customers.map(customer=>({id:customer._id.toString(),name:String(customer.name),code:String(customer.code)}))}/></div>
